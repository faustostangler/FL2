...
class CompanyRepositoryPort(ABC):
    ...
    def save_all(self, items: List[CompanyDTO]) -> None:
        ...
        import logging; logging.basicConfig(level=logging.DEBUG); logging.debug("company_repository_port.CompanyReposirotyPort(ABC).save_all()")
        raise NotImplementedError

    @abstractmethod
    def get_all_primary_keys(self) -> Set[str]:
        """Return all stored CVM codes."""
        import logging; logging.basicConfig(level=logging.DEBUG); logging.debug("company_repository_port.CompanyReposirotyPort(ABC).get_all_primary_keys()")
        raise NotImplementedError

    @abstractmethod
    def get_by_id(self, id: str) -> CompanyDTO:
        import logging; logging.basicConfig(level=logging.DEBUG); logging.debug("company_repository_port.CompanyReposirotyPort(ABC).get_by_id()")
